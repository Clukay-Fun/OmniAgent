# OmniAgent

**多模块智能 Agent 平台**，采用微服务架构，支持多种 Agent 实例接入不同的通讯渠道（飞书、微信、Web 等）。

---

## 📊 当前开发进度

> 最后更新：2026-02-06

### 已完成功能

| 功能模块 | 状态 | 说明 |
|----------|------|------|
| **Webhook 接入** | ✅ 完成 | 飞书消息回调、验签、去重 |
| **意图识别** | ✅ 完成 | 规则优先 + LLM 兜底 |
| **QuerySkill** | ✅ 完成 | 案件查询、关键词/日期范围搜索 |
| **SummarySkill** | ✅ 完成 | 查询结果汇总 |
| **ReminderSkill** | ✅ 完成 | 提醒 CRUD + 定时推送 |
| **ChitchatSkill** | ✅ 完成 | 问候、帮助、敏感拒答 |
| **人员字段搜索** | ✅ 完成 | 使用 open_id 精确匹配主办律师 |
| **表格别名识别** | ✅ 完成 | 支持"案件"→"案件项目总库"映射 |
| **MCP 工具层** | ✅ 完成 | 多维表格增删查改 |

### 开发中功能

| 功能模块 | 状态 | 说明 |
|----------|------|------|
| **CreateSkill** | 🚧 开发中 | 引导式创建案件 |
| **UpdateSkill** | 🚧 开发中 | 字段更新 |
| **DeleteSkill** | 🚧 开发中 | 二次确认删除 |
| **开庭日提醒** | 🚧 开发中 | 自动扫描并推送 |

### 待开发功能

| 功能模块 | 状态 | 说明 |
|----------|------|------|
| **向量记忆** | 📋 规划中 | Chroma + 语义检索 |
| **Web Agent** | 📋 规划中 | Web 聊天界面 |
| **WeChat Agent** | 📋 规划中 | 微信机器人 |

---

## 🏗️ 系统架构

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                              通讯渠道                                        │
│         ┌──────────┐    ┌──────────┐    ┌──────────┐                        │
│         │  飞书    │    │  微信    │    │   Web    │   ...                  │
│         └────┬─────┘    └────┬─────┘    └────┬─────┘                        │
└──────────────┼───────────────┼───────────────┼──────────────────────────────┘
               │               │               │
               ▼               ▼               ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                            Agent 实例层                                      │
│   ┌────────────────┐  ┌────────────────┐  ┌────────────────┐                │
│   │ Feishu Agent   │  │ WeChat Agent   │  │  Web Agent     │   ...          │
│   │ (律师助手)      │  │ (待开发)        │  │ (待开发)        │                │
│   └────────┬───────┘  └────────────────┘  └────────────────┘                │
└────────────┼────────────────────────────────────────────────────────────────┘
             │
             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                            共享基础设施                                       │
│   ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐    │
│   │ Skill 系统   │  │ Soul 系统    │  │ Memory 系统  │  │ MCP Server   │    │
│   └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘    │
└─────────────────────────────────────────────────────────────────────────────┘
             │                                               │
             ▼                                               ▼
      ┌──────────────┐                              ┌──────────────┐
      │  LLM 服务    │                              │  数据存储     │
      │ (SiliconFlow)│                              │ (PostgreSQL) │
      └──────────────┘                              └──────────────┘
```

### 双服务分离架构

```
┌─────────────────────────────────────────────────────────────────────┐
│                     Feishu Agent（端口 8080）                        │
│   持有：FEISHU_BOT_* 凭证                                           │
│   职责：Webhook处理、消息发送、意图解析、技能执行                      │
└───────────────────────────────────┬─────────────────────────────────┘
                                    │ HTTP 调用
                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│                   MCP Feishu Server（端口 8081）                     │
│   持有：FEISHU_DATA_* 凭证                                           │
│   职责：多维表格发现、搜索、CRUD、表结构缓存                           │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 📁 目录结构

```
OmniAgent/
├── agent/                          # Agent 实例目录
│   └── feishu-agent/               # 飞书 Agent
│       ├── src/
│       │   ├── api/                # API 入口（webhook）
│       │   ├── core/               # 核心模块
│       │   │   ├── orchestrator.py # 编排器
│       │   │   ├── intent/         # 意图解析
│       │   │   ├── router/         # 技能路由
│       │   │   └── skills/         # 技能实现
│       │   ├── llm/                # LLM 客户端
│       │   ├── mcp/                # MCP 客户端
│       │   └── user/               # 用户身份管理
│       ├── config/                 # 配置文件
│       └── workspace/              # 运行时数据
│
├── mcp/                            # MCP 工具服务
│   └── mcp-feishu-server/          # 飞书 MCP Server
│       ├── src/
│       │   ├── tools/              # 工具实现
│       │   └── server/             # HTTP 服务
│       └── config.yaml             # 配置文件
│
├── docs/                           # 文档（已整合至此）
├── monitoring/                     # 监控配置
└── docker-compose.yml              # Docker 编排
```

---

## 🚀 快速开始

### 环境要求

- Python 3.11+
- PostgreSQL 14+（Reminder 需要）
- 飞书开放平台应用

### 1. 启动 MCP Server

```bash
cd mcp/mcp-feishu-server
pip install -r requirements.txt
cp .env.example .env  # 配置飞书凭证
python run_server.py
```

### 2. 启动 Feishu Agent

```bash
cd agent/feishu-agent
pip install -r requirements.txt
cp .env.example .env  # 配置飞书凭证和 LLM
python run_server.py
```

### 3. 配置飞书 Webhook

在飞书开放平台配置事件订阅地址：`http://your-server:8080/feishu/webhook`

---

## ⚙️ 配置说明

### MCP Server 环境变量

```env
# 飞书应用凭证
FEISHU_DATA_APP_ID=cli_xxx
FEISHU_DATA_APP_SECRET=xxx

# 多维表格配置
BITABLE_DOMAIN=xxx
BITABLE_APP_TOKEN=xxx
BITABLE_TABLE_ID=xxx
BITABLE_VIEW_ID=xxx
```

### Feishu Agent 环境变量

```env
# 飞书机器人凭证
FEISHU_BOT_APP_ID=cli_xxx
FEISHU_BOT_APP_SECRET=xxx
FEISHU_BOT_VERIFICATION_TOKEN=xxx

# LLM 配置
LLM_API_KEY=sk-xxx
LLM_MODEL=internlm/internlm2_5-7b-chat
LLM_API_BASE=https://api.siliconflow.cn/v1

# MCP Server 地址
MCP_SERVER_BASE=http://localhost:8081
```

---

## 🔧 MCP 工具列表

| 工具名 | 功能 | 状态 |
|--------|------|------|
| `feishu.v1.bitable.list_tables` | 列出表格 | ✅ |
| `feishu.v1.bitable.search` | 通用搜索 | ✅ |
| `feishu.v1.bitable.search_keyword` | 关键词搜索 | ✅ |
| `feishu.v1.bitable.search_exact` | 精确匹配 | ✅ |
| `feishu.v1.bitable.search_person` | 人员字段搜索 | ✅ |
| `feishu.v1.bitable.search_date_range` | 日期范围搜索 | ✅ |
| `feishu.v1.bitable.record.get` | 获取单条记录 | ✅ |
| `feishu.v1.bitable.record.create` | 创建记录 | ✅ |
| `feishu.v1.bitable.record.update` | 更新记录 | ✅ |
| `feishu.v1.bitable.record.delete` | 删除记录 | ✅ |
| `feishu.v1.doc.search` | 文档搜索 | ✅ |

---

## 📊 监控与可视化

- **健康检查**：`GET /health`
- **指标端点**：`GET /metrics`（Prometheus 格式）
- **可用指标**：
  - `feishu_agent_requests_total` - 请求总数
  - `feishu_agent_skill_executions_total` - 技能执行次数
  - `feishu_agent_skill_execution_duration_seconds` - 执行延迟

### 启动监控

```bash
# Linux/Mac
./monitoring/run_monitoring.sh

# Windows
./monitoring/run_monitoring.ps1
```

---

## 🔍 故障排查

### 常见问题

| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 403 Forbidden | 应用权限不足 | 在飞书开放平台配置 `bitable:app` 权限 |
| WrongViewId | View ID 配置错误 | 清空或更新 `BITABLE_VIEW_ID` |
| InvalidFilter | 字段类型不匹配 | 人员字段使用 `search_person` 工具 |
| 无法识别表格 | 别名未配置 | 更新 `skills.yaml` 中的 `table_aliases` |

### 常用命令

```bash
# MCP 健康检查
curl http://localhost:8081/health

# 查看工具列表
curl http://localhost:8081/mcp/tools

# 查看表格字段
curl http://localhost:8081/bitable/fields

# 查看服务日志
docker compose logs --tail=200 feishu-agent
docker compose logs --tail=200 mcp-feishu-server
```

---

## 📚 相关文档

- [测试用例](./TEST.md)
- [MCP Server 详细文档](./mcp/mcp-feishu-server/README.md)
- [Feishu Agent 详细文档](./agent/feishu-agent/README.md)

---

## 📄 License

MIT License
