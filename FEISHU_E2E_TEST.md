...
1. 日期字段 `开庭日` 为文本字段，当前数据为空或格式不匹配，日期查询返回空结果。
2. 样例数据中未包含“张三”，导致 T07 关键词查询无结果。
3. Reminder 列表/更新依赖数据库，当前未配置 DB（`当前未配置数据库，无法更新提醒。`）。
4. 记忆写入未落盘（`/app/workspace/users/test_user/memory.md` 不存在）。
5. 向量记忆未启用（Embedding API 配置缺失）。

## 五、结论

- 已完成 T01-T22 复测，T23 未执行。
- 当前通过 7 项（T01/T03/T04/T05/T06/T08/T12）。
- Reminder 列表/更新/定时推送与 Memory/Soul 相关用例仍需 DB 与数据完善后复测。
