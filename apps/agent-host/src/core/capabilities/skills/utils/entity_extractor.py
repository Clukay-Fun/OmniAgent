...
class EntityExtractor:
    ...
    async def extract_exact_match_field(
        ...
        if not available_fields:
            available_fields = ["编号", "ID", "名称"]

        system_prompt = (
            "你是一个多维表格数据查询实体提取器。\n"
            "你的任务是从用户的查询语句中，提取出用于精确定位一条记录的关键标识（如编号、主键或特定名称）。\n"
            f"以下是当前表格中可用于精确匹配的目标字段名：{', '.join(available_fields)}。\n"
            "你需要：\n"
            "1. 判断用户想要通过哪个字段进行查询。\n"
            "2. 提取出对应的确切值（去掉末尾无意义的口语化描述词）。\n"
    ...
    async def parse_update_fields(
        ...
        system_prompt = (
            ...
            "2. 不要提取用于定位该记录的条件（定位条件通常作为定语，不是要写入的新值）。\n"
            "3. 只返回一个纯 JSON 对象，键为要更新的字段名，值为新值。例如 `{\"状态\": \"已完成\"}`。\n"
            "如果没有找到明确的更新意图，返回 `{}`。"
        )
